...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # tmp dir, and Feishu/Lark shows as "not configured").
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    # can fight over the same Feishu/Lark credentials on every gateway startup.
...
def _all_platforms() -> list[dict]:
    """Return the Feishu/Lark platform entry for setup menus."""
    return [dict(p) for p in _PLATFORMS if p["key"] == "feishu"]
...
def _setup_standard_platform(platform: dict):
    """Interactive setup for a Feishu/Lark platform schema."""
...
def _configure_platform(platform: dict) -> None:
    ...
    This build exposes only the built-in Feishu / Lark setup entry.
    """
    entry = platform.get("_registry_entry")
...
def gateway_setup():
    ...
    # Consider Feishu / Lark configured once the user has made meaningful
    # progress.

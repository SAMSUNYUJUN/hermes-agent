...
class AIAgent:
    ...
    _SKILL_REVIEW_PROMPT = (
        "Generic single-conversation skill generation is disabled. Do not "
        "create, patch, edit, or delete skills from this review. Website "
        "search skills are generated only by the DTC site-search background "
        "learner after the configured number of successful cleaned exploration "
        "chains for the same site URL. Say 'Nothing to save.' and stop."
    )

    _COMBINED_REVIEW_PROMPT = (
        "Review the conversation above for memory only.\n\n"
        "Did the user reveal persona, desires, preferences, personal details, "
        "or expectations about how you should behave? Save durable facts with "
        "the memory tool when appropriate.\n\n"
        "Do not create, patch, edit, or delete skills from this review. "
        "Generic single-conversation skill generation is disabled. Website "
        "search skills are generated only by the DTC site-search background "
        "learner after the configured number of successful cleaned exploration "
        "chains for the same site URL.\n\n"
        "If nothing is worth saving to memory, say 'Nothing to save.' and stop."
    )
    ...
    def _build_system_prompt_parts(self, system_message: str = None) -> Dict[str, str]:
        ...
        if "dtc_site_search_context" in self.valid_tool_names and "dtc_site_search_record" in self.valid_tool_names:
            tool_guidance.append(
                "For TikTok SKU plus DTC independent-site search tasks, web_search is "
                "not the starting tool. Required order: (1) call tiktok_sku_lookup(sku_id); "
                "(2) call dtc_site_search_context(site_url); if it returns "
                "has_tool=true, call dtc_site_search_tool first. If that tool "
                "returns success=true, do not load the site skill; answer from "
                "the structured candidates/evidence and record route_used='tool_first' "
                "with tool_success=true. Only if dtc_site_search_tool returns "
                "success=false should you fall back to skill_view(name=skill_view_name) "
                "when has_skill=true. If has_tool=false and has_skill=true, call "
                "skill_view(name=skill_view_name) and follow the loaded site skill's "
                "Minimal Successful Path exactly, including any canonical redirect "
                "target or category URL. When a site skill is used, it overrides the "
                "generic requirement to start at the user-provided URL; do not re-explore "
                "URLs, search boxes, buttons, snapshots, or clicks listed in the skill's "
                "Do Not Do section. If dtc_site_search_context returns has_tool=false "
                "and has_skill=false, call "
                "browser_navigate with the exact user-provided DTC URL and inspect "
                "menus, search boxes, product grids, collection pages, and all-products "
                "pages with browser_snapshot/browser_click/browser_type/browser_press. Only "
                "after those browser steps fail or reveal a related catalog domain may "
                "you use web_search. If browser_navigate is unavailable or fails, use "
                "web_extract on the exact DTC URL before any search-engine query. "
                "If asked to judge whether two products are the same and both sides have "
                "images, use the available vision capability to compare the product "
                "images; if either side has no image, skip visual comparison and rely on "
                "textual evidence. Do not decide the final same-item match unless "
                "explicitly asked. After "
                "one complete site exploration, call dtc_site_search_record exactly once "
                "so the background learner can clean the path and eventually promote "
                "site-specific search experience into a skill."
            )

...
def resolve_channel_name(platform_name: str, name: str) -> Optional[str]:
    ...
    Matching is case-insensitive; exact IDs and unambiguous name prefixes work.
    """
    directory = load_directory()
    ...
    # 0. Exact ID match — case-sensitive, no normalization.
    raw = name.strip()
    for ch in channels:
        if ch.get("id") == raw:
            return ch["id"]
...
def format_directory_for_display() -> str:
    ...
    lines.append('Bare platform name "feishu" sends to the Feishu home channel.')
